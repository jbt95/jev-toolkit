...
| claude-code | `claude-code` | `~/.claude/CLAUDE.md` ("Jev decision routing") | plugin `integrations/claude-code/` → `UserPromptSubmit` hook runs `jev hook prompt`; `Stop` hook runs failure triage (warn-only) | `claude plugin marketplace add jbt95/jev-toolkit` then `claude plugin install jev-toolkit@jev-toolkit` |
...
| omp | `omp` | `~/.omp/agent/RULES.md` ("Jev decision routing") | plugin `integrations/omp/` → same `before_agent_start` handler; OMP vendors the Pi API, and the root `package.json` `pi` manifest wires the extension and skill | `omp plugin install <clone>` then `omp plugin doctor` (verified live: recall fired on a `versus` prompt with the plugin as the only wiring) |
...
`claude plugin marketplace add jbt95/jev-toolkit` plus
...
`omp plugin install <clone>` wires recall and routing in one
...
pi session lost 3 of 4 `typesafe_ask` attempts to
